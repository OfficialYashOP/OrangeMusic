...
export default function AlbumDetailScreen({ route, navigation }: any) {
    ...
    const { albumName, albumImage, albumArtist, albumYear } = route.params;
    ...
    useEffect(() => {
        (async () => {
            try {
                const result = await searchSongs(`${albumName} ${albumArtist ?? ''}`, 0, 20);
    ...
    return (
        <View style={[styles.container, { backgroundColor: c.bg, paddingTop: insets.top }]}>
            ...
            <View style={styles.artWrap}>
                <Image source={{ uri: albumImage }} style={[styles.art, { backgroundColor: c.card }]} />
            </View>

            <Text style={[styles.name, { color: c.text }]}>{albumName}</Text>
            <Text style={[styles.meta, { color: c.muted }]}>
                {albumArtist ?? 'Various'}  |  {albumYear ?? ''}  |  {songs.length} songs
            </Text>
